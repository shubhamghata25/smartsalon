...
router.post("/", authenticate, requireAdmin, async (req, res) => {
  const { service_id, name, price, duration, description, image_url, sort_order } = req.body;
  if (!service_id || !name || price === undefined)
    return res.status(400).json({ error: "service_id, name, price required" });
  try {
    const { rows } = await db.query(
      `INSERT INTO sub_services (service_id,name,price,duration,description,image_url,sort_order)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [service_id, name, price, duration||null, description||null, image_url||null, sort_order||0]
    );
...
router.patch("/:id", authenticate, requireAdmin, async (req, res) => {
  const { name, price, duration, description, image_url, sort_order, is_active } = req.body;
  try {
    const { rows } = await db.query(
      ...
      [name, price, duration, description, image_url, sort_order, is_active, req.params.id]
    );
